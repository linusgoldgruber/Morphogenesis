...
void FluidSystem::InitializeOpenCL() {      //used for load_sim
    ...
    queue = clCreateCommandQueueWithProperties(clContext, devices[0], properties, &err);
    if (err != CL_SUCCESS) {
    ...
    FILE *fp = fopen("fluid_system_opencl.cl", "r");
    if (!fp) {
    ...
    cl_mem clFBuf = clCreateBuffer(clContext, CL_MEM_READ_WRITE, sizeof(m_Fluid), NULL, ret);
    //clCheck (clFBuf , "LoadKernel", "cuModuleGetGlobal", "clFBuf",    mbDebug);   // Returns a global pointer (clFBuf) from a module  (m_program).

    cl_mem clFTemp = clCreateBuffer(clContext, CL_MEM_READ_WRITE, sizeof(m_FluidTemp), NULL, ret);
